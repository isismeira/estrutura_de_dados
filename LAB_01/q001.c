...
void misc1(TAB* a) {
    int resultado = recursao_nivel(a, 0);
    printf("%i", resultado);
}
...
int recursao_nivel(TAB* a, int nivel) {
    ...
    if(nivel % 2 == 0) {
        return -(a->info) + recursao_nivel(a->esq, nivel + 1) + recursao_nivel(a->esq, nivel + 1);
    } else {
        return (a->info) - recursao_nivel(a->esq, nivel + 1) - recursao_nivel(a->dir, nivel+1);
    }
}
