...
int dfs_tem_caminho(TG* g, int no_atual, int y) {
    if (no_atual == y) {
        return 1;
    }
    visitado[indice_no(g, atual)] = 1;
    TG *atual = TG_busca_no(g, no_atual);
    for (TVIZ *v = no_atual->prim_viz; v; v = v->prox_viz) {
        idx_viz = indice_no(g, v->id_viz);
        if(!visitado[idx_viz]) {
            if (dfs_tem_caminho(TG* g, int no_atual, int y)) {
                return 1;
            }
        }
    }
...
int caminho(TG *g, int x, int y){
    if(!g) return 0;
    int n = conta_nos(g);
    TG* no_x = TG_busca_no(g,x);
    for (TVIZ *v = no_atual->prim_viz; v; v = v->prox_viz) {
        int* visitado = (int*)calloc(n, sizeof(int));
        visitado[indice_no(g, no_x)] = 1;
        if(dfs_tem_caminho(g, v->id_viz, y))
            return 1
    }
    return 0;
}