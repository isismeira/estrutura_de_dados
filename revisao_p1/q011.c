...
// Faça uma função que descubra se um grafo não direcionado é cíclico - int dir_ciclico(TG *g).
...
// DFS recursiva para detectar ciclo
int dfs_ciclo(TG *g, int atual, int pai, int *visitado) {
    visitado[indice_no(g, atual)] = 1;

    TG *no_atual = TG_busca_no(g, atual);
    for (TVIZ *v = no_atual->prim_viz; v; v = v->prox_viz) {
        int idx_viz = indice_no(g, v->id_viz);
        if (!visitado[idx_viz]) {
            if (dfs_ciclo(g, v->id_viz, atual, visitado))
                return 1;
        }
        else if (v->id_viz != pai) {
            return 1;
        }
    }
...
int dir_ciclico(TG *g) {
    ...
    TG *aux = g;
    while (aux) {
        int idx = indice_no(g, aux->id_no);
        if (!visitado[idx]) {
            if (dfs_ciclo(g, aux->id_no, -1, visitado)) {
                free(visitado);
                return 1;
            }
        }
